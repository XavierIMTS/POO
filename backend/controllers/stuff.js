...
exports.modifyThing = (req, res, next) => {
  ...
  } : { ...req.body }; // Sinon, récupération des données sans modification de l'image
  ...
  Thing.findOne({_id: req.params.id}) // Recherche de l'objet dans la base de données
    .then((thing) => {
      if (thing.userId != req.auth.userId) { // Vérification que l'utilisateur authentifié est bien le propriétaire de l'objet
        ...
        Thing.updateOne({ _id: req.params.id}, { ...thingObject, _id: req.params.id}) // Sinon, mise à jour de l'objet dans la base de données
    ...
    .catch((error) => {
      res.status(400).json({ error }); // En cas d'erreur lors de la recherche de l'objet dans la base de données, renvoi de l'erreur
    });
};
